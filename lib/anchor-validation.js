...
export function promptContainsAnchor(prompt, keywords) {
  ...
  return keywords.some((kw) => {
    if (!kw) return false;
    const variants = [kw];
    if (!kw.endsWith('s')) variants.push(`${kw}s`);
    if (kw.endsWith('s') && kw.length > 3) variants.push(kw.slice(0, -1));
    return variants.some((variant) => {
      if (variant.includes(' ')) return p.includes(variant);
      return new RegExp(`\\b${escapeRegex(variant)}\\b`, 'i').test(p);
    });
  });
}
 ...
 * @param {{ expectedCount?: number, requireAnchorField?: boolean }} [opts]
 */
export function validateConcepts(lockedAnchor, concepts, opts = {}) {
  const expectedCount = opts.expectedCount ?? 3;
  const requireAnchorField = opts.requireAnchorField ?? true;
  ...
  if (concepts.length !== expectedCount) {
    errors.push(`Expected exactly ${expectedCount} concepts, got ${concepts.length}`);
  }
  ...
  for (let i = 0; i < concepts.length; i++) {
    ...
    if (requireAnchorField && !conceptAnchor) {
      errors.push(`Concept ${i + 1}: missing anchor field`);
    } else if (conceptAnchor && conceptAnchor !== normalizedLocked) {
      const anchorsMatch =
